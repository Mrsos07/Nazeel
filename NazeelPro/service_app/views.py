...
def edit_items(request: HttpRequest, main_services_id):
    if request.method == 'POST':
        ...
        new_sub_service = SubService(main_service=main_services_object, name_service=request.POST["name_service"],
                                     catogory=request.POST["catogory"], price=request.POST["price"],
                                     image=request.FILES["image"], delivery_time=request.POST["delivery_time"])
    ...
    return render(request, "service_app/edit_items.html",
                  {"delete_items": delete_items, "main_services": main_services})
...
def order_request(request: HttpRequest, main_services_id):
    ...
    return render(request, "service_app/order_request.html",
                  {'main_services': main_services, 'sub_service': sub_service})
